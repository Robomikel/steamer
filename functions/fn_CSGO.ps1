Function New-LaunchScriptcsgoserverPS {
        ...
        * mg_armsrace                           * mg_op_op08                                                                    " -ForegroundColor Yellow
        ...
        Add-Content -Path $global:currentdir\$global:server\Launch-$global:server.ps1 -Value "start-process cmd `"/c zcsgo.exe -game csgo -console -usercon -strictportbind -ip `${global:IP} -port `$global:PORT +clientport  `$global:CLIENTPORT +tv_port `$global:SOURCETVPORT +sv_setsteamaccount `"`$GSLT`" -tickrate `$global:TICKRATE +map `$global:MAP -maxplayers_override `$global:MAXPLAYERS +mapgroup `$global:MAPGROUP +game_type `$global:GAMETYPE +game_mode `$global:GAMEMODE +host_workshop_collection `${wscollectionid} +workshop_start_map `${WSSTARTMAP} -authkey `${WSAPIKEY} -nobreakpad +net_public_adr ${global:EXTIP} -condebug`" -Wait -NoNewWindows"
