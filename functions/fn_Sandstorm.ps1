Function New-LaunchScriptInsserverPS {
        ...
        If ($global:SERVERPASSWORD -ne "") {
                Write-Host "**** Creating start vars ******" -ForegroundColor Magenta
                Add-Content $global:currentdir\$global:server\Variables-$global:server.ps1 "`$global:launchParams = @(`"`$global:EXE `${global:MAP}?Scenario=`${global:SCENARIO}?MaxPlayers=`${global:MAXPLAYERS}?password=`${global:SERVERPASSWORD} -Port=`${global:PORT} -QueryPort=`${global:QUERYPORT} -log -hostname=$q`"`${global:HOSTNAME}$q`"`")"
        }
        Else {
                Add-Content -Path $global:currentdir\$global:server\Variables-$global:server.ps1 -Value "`$global:launchParams = @(`"`$global:EXE `${global:MAP}?Scenario=`${global:SCENARIO}?MaxPlayers=`${global:MAXPLAYERS} -Port=`${global:PORT} -QueryPort=`${global:QUERYPORT} -log -hostname=$q`"`${global:HOSTNAME}$q`"`")"       
        }
