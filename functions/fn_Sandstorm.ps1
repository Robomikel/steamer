Function New-LaunchScriptInssserverPS {
        ...
        If ($global:SERVERPASSWORD -eq ""){Add-Content -Path $global:currentdir\$global:server\Launch-$global:server.ps1 -Value "Start-Process -FilePath InsurgencyServer.exe -ArgumentList (`"${global:MAP}?Scenario=${global:SCENARIO}?MaxPlayers=${global:MAXPLAYERS} -Port=${global:PORT} -QueryPort=${global:QUERYPORT} -log`")"}else{
        Add-Content -Path $global:currentdir\$global:server\Launch-$global:server.ps1 -Value "Start-Process -FilePath InsurgencyServer.exe -ArgumentList (`"${global:MAP}?Scenario=${global:SCENARIO}?MaxPlayers=${global:MAXPLAYERS}?password=${global:SERVERPASSWORD} -Port=${global:PORT} -QueryPort=${global:QUERYPORT} -log`")"}
