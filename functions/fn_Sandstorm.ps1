Function New-LaunchScriptInssserverPS {
        ...
        If ($global:SERVERPASSWORD -eq ""){Add-Content -Path $global:currentdir\$global:server\Launch-$global:server.ps1 -Value "start-process cmd `"/c $global:currentdir\$global:server\InsurgencyServer.exe `${global:MAP}?Scenario=`${global:SCENARIO}?MaxPlayers=`${global:MAXPLAYERS} -Port=`${global:PORT} -QueryPort=`${global:QUERYPORT} -log -hostname='`${global:HOSTNAME}'`" -Wait -NoNewWindow"}Else{
        Add-Content -Path $global:currentdir\$global:server\Launch-$global:server.ps1 -Value "start-process cmd `"/c $global:currentdir\$global:server\InsurgencyServer.exe `${global:MAP}?Scenario=`${global:SCENARIO}?MaxPlayers=`${global:MAXPLAYERS}?password=`${global:SERVERPASSWORD} -Port=`${global:PORT} -QueryPort=`${global:QUERYPORT} -log -hostname='`${global:HOSTNAME}'`" -Wait -NoNewWindow"}
