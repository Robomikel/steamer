...
Function New-LaunchScriptRustPS {
        ...
        $q = "``"
        # - - - - - - - - - - - - -
        $global:SEED = Get-Random -Minimum 1 -Maximum 2147483647
        ...
        if (($global:SEED = Read-Host -Prompt (Write-Host "Input Server seed,Press enter to accept default value [$global:SEED]: " -ForegroundColor Cyan -NoNewline)) -eq '') { $global:SEED = "$global:SEED" }else { $global:SEED }
        ...
        #Add-Content -Path $global:currentdir\$global:server\Launch-$global:server.ps1 -Value "Start-Process cmd `"/c RustDedicated.exe -batchmode +server.ip `${global:IP}  +server.port `${global:PORT} +server.tickrate `${global:TICKRATE} +server.hostname $q`"`${global:HOSTNAME}$q`" +server.maxplayers `${global:MAXPLAYERS} +server.worldsize `${global:WORLDSIZE} +server.saveinterval `${global:SAVEINTERVAL} +rcon.web `${global:RCONWEB} +rcon.ip 0.0.0.0 +rcon.port `${global:RCONPORT} +rcon.password `${global:RCONPASSWORD} -logfile $global:currentdir\$global:server\Serverlog-$date.log`""
        Add-Content -Path $global:currentdir\$global:server\Launch-$global:server.ps1 -Value "If ( !$? ) {Start-Process -FilePath cmd -ArgumentList (`"/c RustDedicated.exe -batchmode +server.ip `${global:IP}  +server.port `${global:PORT} +server.tickrate `${global:TICKRATE} +server.maxplayers `${global:MAXPLAYERS} +server.worldsize `${global:WORLDSIZE} +server.saveinterval `${global:SAVEINTERVAL} +rcon.web `${global:RCONWEB} +rcon.ip 0.0.0.0 +rcon.port `${global:RCONPORT} +rcon.password `${global:RCONPASSWORD} -logfile $global:currentdir\$global:server\Serverlog-$date.log`")"} 
        ...
        # add to server.cfg                   server.hostname "server"
