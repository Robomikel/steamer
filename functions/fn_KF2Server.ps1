Function New-LaunchScriptKF2serverPS {
    ...
    Add-Content -Path $global:currentdir\$global:server\Launch-$global:server.ps1 -Value "start-process cmd `"/c KFServer.exe `${global:MAP}?Game=`${global:GAMEMODE}?Difficulty=`${global:DIFF}? -Port=`${global:PORT} -QueryPort=`${global:QUERYPORT}`" -Wait -NoNewWindow"
