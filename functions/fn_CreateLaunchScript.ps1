...
Function New-LaunchScriptceserverPS {
        ...
        Add-Content -Path $global:currentdir\$global:server\Launch-$global:server.ps1 -Value "Write-Host `"****   Server Starting  ****`" -ForegroundColor Magenta -BackgroundColor Black"
        Add-Content -Path $global:currentdir\$global:server\Launch-$global:server.ps1 -Value "Set-Location $global:currentdir\$global:server\"
        Add-Content -Path $global:currentdir\$global:server\Launch-$global:server.ps1 -Value "start-process cmd `"/c ConanSandboxServer.exe -log  -MaxPlayers=`${global:MAXPLAYERS} -Port=`${global:PORT} -QueryPort=`${global:QUERYPORT} -RconEnabled=1 -RconPassword=`${global:RCONPASSWORD} -RconPort=`${global:RCONPORT}`"  -NoNewWindows"
}
...
Function  New-LaunchScriptavserverPS {
        ...
        Add-Content -Path $global:currentdir\$global:server\Launch-$global:server.ps1 -Value "start-process `"cmd`" `"/c start AvorionServer.exe --server-name `${global:HOSTNAME} --galaxy-name `${global:GALAXYNAME} --admin `${global:steamID64} --difficulty `${global:DIFF} --max-players `${global:MAXPLAYERS}`"  -NoNewWindows"
...
Function New-LaunchScriptforestserverPS {
        ...
        Add-Content -Path $global:currentdir\$global:server\Launch-$global:server.ps1 -Value "Write-Host `"****   Server Starting  ****`" -ForegroundColor Magenta -BackgroundColor Black"
        Add-Content -Path $global:currentdir\$global:server\Launch-$global:server.ps1 -Value "Set-Location $global:currentdir\$global:server\"
        Add-Content -Path $global:currentdir\$global:server\Launch-$global:server.ps1 -Value "start-process cmd `"/c TheForestDedicatedServer.exe -serverip `${global:IP} -serversteamport `${global:STEAMPORT} -servergameport `${global:PORT} -serverqueryport `${global:QUERYPORT} -servername `"`${global:HOSTNAME}`" -serverplayers `${global:MAXPLAYERS} -difficulty Normal -configfilepath $global:currentdir\$global:server\SKS\TheForestDedicatedServer\ds\server.cfg -inittype Continue -slot 4 -batchmode -nographics`"  -NoNewWindows" # -nosteamclient"
...
Function New-LaunchScriptAoCserverPS {
        ...
        Add-Content -Path $global:currentdir\$global:server\Launch-$global:server.ps1 -Value "Write-Host `"****   Server Starting  ****`" -ForegroundColor Magenta -BackgroundColor Black"
        Add-Content -Path $global:currentdir\$global:server\Launch-$global:server.ps1 -Value "Set-Location $global:currentdir\$global:server\"
        Add-Content -Path $global:currentdir\$global:server\Launch-$global:server.ps1 -Value "start-process cmd `"/c aoc.exe -console -game ageofchivalry -secure +map `${global:MAP} -autoupdate +log on +maxplayers `${global:MAXPLAYERS} -port `${global:PORT} +ip `${global:IP} +exec server.cfg`"  -NoNewWindows"
}
...
Function New-LaunchScriptasserverPS {
        ...
        Add-Content -Path $global:currentdir\$global:server\Launch-$global:server.ps1 -Value "Write-Host `"****   Server Starting  ****`" -ForegroundColor Magenta -BackgroundColor Black"
        Add-Content -Path $global:currentdir\$global:server\Launch-$global:server.ps1 -Value "Set-Location $global:currentdir\$global:server\"
        Add-Content -Path $global:currentdir\$global:server\Launch-$global:server.ps1 -Value "start-process cmd `"/c asds.exe -console -game swarm +map lobby -maxplayers 4 -autoupdate`"  -NoNewWindows"
}
